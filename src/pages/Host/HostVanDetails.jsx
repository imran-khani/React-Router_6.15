import { useEffect, useState } from "react";
import { BsArrowLeftShort } from "react-icons/bs";
import { Link, useParams, useNavigate, Outlet } from "react-router-dom";
import data from "../../Data.json";

const HostVanDetails = () => {
  const { id } = useParams();
  const [currentVan, setCurrentVan] = useState(null);
  useEffect(() => {
    const findVan = data.find((van) => van.id === id);
    setCurrentVan(findVan);
  }, [id]);
  const navigate = useNavigate();
  if (currentVan === null) {
    return <h1>Loading...</h1>;
  }
  return (
    <div className="p-5 ">
      <div>
        <Link onClick={() => navigate(-1)} className="flex  gap-2">
          <button className="text-2xl">{<BsArrowLeftShort />} </button>
          <span>Back to all vans</span>
        </Link>
      </div>

      {
        <div className="flex mt-10 p-5 bg-white flex-col">
          <div className="flex ml-3">
            <img src={currentVan.imageUrl} className="w-40 h-40" alt="" />
            <div className="flex flex-col ms-5 mt-5 w-full">
              <button className="px-5 py-2 bg-orange-500 text-white rounded-md opacity-90 self-start mb-5">
                {currentVan.type}
              </button>
              <h1 className="text-[26px] font-bold">{currentVan.name}</h1>
              <p className="font-bold">
                {`$${currentVan.price}`}
                <span className="text-gray-500   -ml-1">/day</span>
              </p>
            </div>
          </div>
          <div className="mt-5 p-5 ">
            <nav className="flex gap-5 mb-6">
              <Link to="">Details</Link>
              <Link to="pricing">Pricing</Link>
              <Link to="photos">Photos</Link>
            </nav>
            <Outlet />
          </div>
        </div>
      }
    </div>
  );
};

export default HostVanDetails;
